...
# It uses Meraki Policy Object APIs to delete Group Objects
...
def collect_info():
    ...
    print('It will use Meraki APIs to DELETE Group Objects\n')
    print('********************DEMO********************\n')
    ...
    if json_policy_obj_groups:   # List is not empty - some objects found in Dashboard
        ...
        if delete_obj == "y":
            ...
            for d in json_policy_obj_groups:
                ...
                url = f"/organizations/{org_id}/policyObjects/groups/{policy_obj_group_id}"
                action_payload = {
                    'resource': url,
                    'operation': 'destroy',
                    'body': {}
...
def batch_objects(base_url, api_key, org_id, actions):
    ...
    try:
        ...
        print(f'Action Batch response code: {response.status_code}')  # We want a Status code of 200
